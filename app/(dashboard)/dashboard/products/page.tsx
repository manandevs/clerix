    ...
    <div className="flex flex-col h-[calc(100vh-theme(spacing.24))] px-4 lg:px-6 pb-6 overflow-hidden">
      ...
      <div className="flex items-center gap-3 py-4 shrink-0">
        <div className="relative w-full max-w-sm">
          ...
          <Input placeholder="Search SKU or title..." className="pl-9 bg-zinc-900 border-zinc-800" />
        </div>
        <Select defaultValue="all">
          <SelectTrigger className="w-[150px] bg-zinc-900 border-zinc-800"><SelectValue placeholder="Status" /></SelectTrigger>
          <SelectContent><SelectItem value="all">All Statuses</SelectItem><SelectItem value="active">Active</SelectItem><SelectItem value="draft">Draft</SelectItem></SelectContent>
        </Select>
      </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide pb-12">
        {view === "list" ? (
          <div className="border border-border/50 rounded-xl overflow-hidden bg-zinc-950/50">
                ...
                {MOCK_PRODUCTS.map((p) => (
                  <tr key={p.id} className="hover:bg-zinc-900/50 transition-colors group">
                    ...
                    <td className="px-4 py-3 text-right opacity-0 group-hover:opacity-100 transition-opacity">
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-zinc-400 hover:text-white"><Copy className="w-4 h-4" /></Button>
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-zinc-400 hover:text-white"><Archive className="w-4 h-4" /></Button>
                    </td>
                  </tr>
                ))}
              </tbody>
              ...
              <Card key={p.id} className="bg-zinc-950 border border-zinc-800 hover:border-primary/40 transition-all group overflow-hidden flex flex-col cursor-pointer hover:shadow-[0_0_20px_rgba(220,38,38,0.1)]">
                ...
                <CardContent className="p-4 flex flex-col flex-1">
                  ...
                  <div className="mt-auto pt-4 flex items-center justify-between">
                    ...
                    <span className="text-xs text-zinc-500">{p.stock} units</span>
                  </div>
                </CardContent>
              </Card>
            ))}
