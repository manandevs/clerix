  ...
  return (
    <div className="flex flex-col md:flex-row h-[calc(100vh-theme(spacing.24))] gap-6 px-4 lg:px-6 pb-6">
      ...
      <Card className="flex-1 bg-zinc-950 border-zinc-800 flex flex-col overflow-hidden relative shadow-inner">
        ...
        <CardContent className="flex-1 p-4 overflow-y-auto text-[11px] text-zinc-300 space-y-1.5 scrollbar-hide">
          {logs.map((log, i) => (
            <div key={i} className="animate-in fade-in slide-in-from-left-2">
              <span className="text-primary/80">{new Date().toLocaleTimeString()}</span> {log}
            </div>
          ))}
          <div ref={logRef} />
        </CardContent>
...
<div className="flex-1 flex flex-col gap-6">
  <Card className="bg-card/50 backdrop-blur border-border/50 shrink-0">
    <CardHeader>
      <CardTitle className="flex items-center gap-2 text-zinc-900">
        <Settings2 className="w-5 h-5 text-primary" /> Active Shields
      </CardTitle>
    </CardHeader>
    <CardContent className="space-y-4">
      <div className="flex items-center justify-between">
        <span className="text-sm font-medium text-zinc-800">Cross-site Tracker Blocking</span>
        <Switch defaultChecked />
      </div>
      <div className="flex items-center justify-between">
        <span className="text-sm font-medium text-zinc-800">Canvas Fingerprint Spoofing</span>
        <Switch defaultChecked />
      </div>
      <div className="flex items-center justify-between">
        <span className="text-sm font-medium text-zinc-700">Force WebRTC Leak Protection</span>
        <Switch defaultChecked />
      </div>
    </CardContent>
  </Card>
  ...
  <Card className="bg-card/50 backdrop-blur border-border/50 flex-1 flex flex-col min-h-[200px]">
    <CardHeader>
      <CardTitle className="flex items-center gap-2 text-zinc-900">
        <Shield className="w-5 h-5 text-primary" /> Traffic Neutralized
      </CardTitle>
    </CardHeader>
    <CardContent className="flex-1 p-0 pb-4 pr-4">
       <ResponsiveContainer width="100%" height="100%">
         <BarChart data={barData}>
           <XAxis dataKey="time" tick={{ fontSize: 10, fill: "hsl(var(--foreground)/0.9)" }} tickLine={false} axisLine={false} />
           <Tooltip cursor={{ fill: "hsl(var(--muted)/0.5)" }} contentStyle={{ backgroundColor: "#09090b", borderColor: "#27272a", color: "var-(--ring)" }} />
           <Bar dataKey="trackers" stackId="a" fill="var(--primary)" radius={[0, 0, 4, 4]} />
           <Bar dataKey="ads" stackId="a" fill="hsl(var(--foreground)/0.7)" radius={[4, 4, 0, 0]} />
         </BarChart>
       </ResponsiveContainer>
    </CardContent>
  </Card>
