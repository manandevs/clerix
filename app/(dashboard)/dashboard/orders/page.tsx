      ...
      <div className="flex-1 overflow-x-auto overflow-y-hidden pb-4 scrollbar-hide">
        <div className="flex h-full gap-6 items-start min-w-max">
          ...
          {COLUMNS.map((col) => (
            <div
              key={col.id}
              className="w-[320px] flex flex-col h-full bg-zinc-950 border border-zinc-800 rounded-xl overflow-hidden"
            >
              ...
              <div className="flex-1 overflow-y-auto p-3 space-y-3 scrollbar-hide">
                {col.orders.map((order) => (
                  <Card
                    ...
                    className={`cursor-pointer transition-all hover:-translate-y-1 ${order.urgent
                  ...
                  >
                    <CardHeader className="p-4 pb-2 flex flex-row items-center justify-between space-y-0">
                      ...
                      <div
                        className={`flex items-center gap-1.5 text-xs font-medium px-2 py-0.5 rounded-md ${order.urgent
                    ...
                    <CardContent className="p-4 pt-2">
                      ...
                      <div className="flex items-center justify-between pt-3 border-t border-zinc-800">
                        <div className="flex gap-2">
                          <Badge
                            variant="outline"
                            className="text-[10px] bg-emerald-500/10 text-emerald-400 border-transparent px-1.5"
                          >
                          ...
                          <Badge
                            variant="outline"
                            className="text-[10px] bg-zinc-800 text-zinc-400 border-transparent px-1.5"
                          >
                            <Package className="w-3 h-3 mr-1" /> {order.items} Item
                          </Badge>
                        </div>
