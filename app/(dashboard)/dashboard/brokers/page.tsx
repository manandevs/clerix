            ...
            <Card key={idx} className="bg-card/30 backdrop-blur border-border/50 hover:border-primary/40 transition-colors group cursor-pointer h-fit">
              <CardHeader className="p-4 pb-2 flex flex-row items-center justify-between space-y-0">
                <div className="flex items-center gap-2">
                  <div className="p-1.5 rounded bg-zinc-900 border border-zinc-800 group-hover:border-primary/30"><Building className="w-4 h-4 text-zinc-400 group-hover:text-primary transition-colors" /></div>
                  <h3 className="font-bold">{broker.name}</h3>
                </div>
                <Badge variant="outline" className={`text-[10px] ${broker.risk === 'Critical' ? 'border-red-500/50 text-red-500' : 'border-zinc-700 text-zinc-400'}`}>
                  {broker.risk} Risk
                </Badge>
              </CardHeader>
              <CardContent className="p-4 pt-2">
                ...
                <div className="mt-4 flex items-center justify-between border-t border-border/50 pt-3 opacity-0 group-hover:opacity-100 transition-opacity">
